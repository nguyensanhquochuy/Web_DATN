...
if (isset($_GET['delete'])) {
    $idTopic = $_GET['delete'];
    $delete = 'DELETE FROM chude WHERE MaCD = ? ';
...
if (isset($_GET['searchTopic'])) {
    ...
    $sql = 'SELECT * FROM chude JOIN monhoc on monhoc.MaMH = chude.MaMH
    WHERE (1=1) 
    ';


    if (!empty($id)) {
        $sql .= " AND  chude.MaCD LIKE  '%$id%' ";
    }

    if (!empty($name)) {
        $sql .= " AND  chude.TenCD LIKE  '%$name%' ";
    }

    if (!empty($typeSubject)) {
        $sql .= " AND  chude.MaMH =  '$typeSubject' ";
    }
...
if (isset($_POST['add_topic'])) {
    $nameIns = $_POST['name'];
    $typeSubjectIns = $_POST['typeSubjects'];
    $checkInsert = true;
    if (empty($nameIns)) {
    ...
    $sql = 'SELECT * FROM chude WHERE TenCD = ?';
    $stmt = $conn->prepare($sql);
    ...
    if ($checkInsert) {
        $sql = 'INSERT INTO chude (TenCD, MaMH) values (?, ?)';
        $stmt->prepare($sql);
        $stmt->bind_param('si', $nameIns, $typeSubjectIns);
        if ($stmt->execute()) {
            echo '<script language="javascript">alert("Add Topic successfull!");</script>';
            header("Refresh: 0; URL=topic.php");
        } else {
        ...
        <div class="page-wrapper">
            ...
            <div class="content container-fluid">
                ...
                <div class="page-header">
                    <div class="row align-items-center">
                        ...
                        <div class="col-auto float-right ml-auto">
                            <a href="#" class="btn add-btn" data-toggle="modal" data-target="#add_topic"><i class="fa fa-plus"></i>Thêm chủ đề</a>
                        </div>
                    </div>
                </div>
                <!-- /Page Header -->
                ...
                <form method="GET" action="topic.php" id="search_Tutor">
                    ...
                    <div class="row filter-row" style="justify-content: center;">
                        <div class="col-sm-6 col-md-3">
                            <div class="form-group form-focus">
                                <button type="submit" class="btn btn-success btn-block" name="searchTopic">Tìm kiếm</button>
                            </div>
                        </div>
                        <div class="col-sm-6 col-md-3">
                            ...
                            <table class="table table-striped custom-table datatable">
                                ...
                                <tbody>
                                    ...
                                    ?>
                                            ...
                                            <td class="text-right">
                                                <div class="dropdown dropdown-action">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-right">
                                                        <a class="dropdown-item" href="edit_topic.php?edit=<?= $row['MaCD'] ?>"><i class="fa fa-pencil m-r-5"></i>Edit</a>

                                                        <a class="dropdown-item" href="#" data-toggle="modal" data-target="#delete_topic<?= $row['MaCD'] ?>"><i class="fa fa-trash-o m-r-5"></i> Delete</a>
                                        ...
                                        <div class="modal custom-modal fade" id="delete_topic<?= $row['MaCD'] ?>" role="dialog">
                                                    ...
                                                    <div class="modal-body">
                                                        ...
                                                        <div class="modal-btn delete-action">
                                                            <form method="post" action="topic.php">
                                                                <div class="row">
                                                                    <div class="col-6">
                                                                        <a href="topic.php?delete=<?= $row['MaCD'] ?>" class="btn btn-primary continue-btn">Delete</a>
                                                                    </div>
                                                                    <div class="col-6">
            ...
            <!-- Add Employee Modal -->
            <div id="add_topic" class="modal custom-modal fade" role="dialog">
                        ...
                        <div class="modal-body">
                            <form method="post" action="topic.php">
                                    ...
                                    <div class="col-sm-6">
                                        ...
                                        <label class="col-form-label">Subject<span class="text-danger">*</span></label>
                                            <select class="select floating select2" name="typeSubjects">
                                                ...
                                                $result = $stmt->get_result();
                                                while ($row = $result->fetch_assoc()) {
                                                    $selected = (isset($typeSubjectIns) && $typeSubjectIns == $row['MaMH']) ? 'selected' : '';
                                                    echo "<option value='{$row['MaMH']}' {$selected}>{$row['TenMH']}</option>";
                                                }
                                                ?>
                                            </select>
                                ...
                                <div class="submit-section">
                                    <button class="btn btn-primary submit-btn" type="submit" name="add_topic">Thêm</button>
                                </div>
                            </form>
                        </div>
                    </div>
