...
<body class="html not-front not-logged-in no-sidebars page-node page-node- page-node-579 node-type-page s">
    ...
    <div class="container p-create-class-body" style="margin-bottom:20px;">
        ...
        if (isset($idClass)) {
            ...
            $stmt->execute();
            $resultClass = $stmt->get_result()->fetch_assoc();

            $nameClass = $resultClass['TenLop'];
            $subjectClass = $resultClass['MaMH'];
            $price = $resultClass['HocPhi1Buoi'];
            $duration = $resultClass['SoGioHoc1Buoi'];
            $phoneClass = $resultClass['DienThoai'];
            $description = $resultClass['NoiDung'];
            $timesPerWeek = $resultClass['SoBuoiHoc1Tuan'];
            $numberStudent = $resultClass['SoHV'];
            $dayStart = $resultClass['NgayBD'];


            $typeClass = $resultClass['MaHT'];
            if ($typeClass != 2) {
                $city = $resultClass['Tinh_TP'];
                $district = $resultClass['Quan_Huyen'];
                $ward = $resultClass['Xa_Phuong'];
                $detailCity = $resultClass['DiaChiCT'];
            }

            $sex = $resultClass['GioiTinhGS'];


            // Lấy dữ liệu lich học của lớp
            ...
            while ($rowTag = $resultClass_Tag->fetch_assoc()) {
                array_push($listTags, $rowTag['MaCD']);
            }
        ...
        if (isset($_POST['submit-info'])) {
            ...
            if ($check) {
                ...
                $stmt->bind_param('siisssdsiiidsssi', $nameClass, $subjectClass, $typeClass, $city, $district, $ward, $duration, $phoneClass, $sex, $numberStudent, $timesPerWeek, $price, $description, $detailCity, $dayStart, $idClass);
                $stmt->execute();

                
                // xoa roi them lại chủ đề cho lớp
                $sql = 'DELETE FROM lophoc_chude WHERE MaLop = ?';
                $stmt = $conn->prepare($sql);
                $stmt->bind_param('i', $idClass);
                $stmt->execute();
                ...
                //  xoa roi them lại lịch học cho lớp
                $sql = 'DELETE FROM lichhoc WHERE MaLop = ?';
                $stmt = $conn->prepare($sql);
                $stmt->bind_param('i', $idClass);
                $stmt->execute();
                            ...
                            <div class="left-class-create bla-common-shadow" style="margin:unset;">
                                ...
                                <div class="group-row">
                                    ...
                                    <div style="text-align:center;padding:10px 0;">
                                        <input name="submit-info" type="submit" class="btn btn-success" style="font-weight:bold;border: none;" value="Cập nhật">
                                    </div>
                                </div>
                            </div>
                        </form>
