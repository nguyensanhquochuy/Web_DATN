$(function () {
    $('#create-class-form').bind('submit', function () {
        //data.append('file', event.blob);
        // Check so dien thoai
        var validate = validate_form();
        if(validate){
          var name_class = $('input[name="name_class"]').val();
          name_class = name_class.trim();
          if (name_class.length < 20) {
            alert("Tóm tắt yêu cầu tìm gia sư phải từ 20 ký tự trở lên.");
            $("#name_class").addClass('missingValue');
        	  return false;
          }else{
            $("#name_class").removeClass('missingValue');
          }
            ...
            if (result.error) {
              ...
              if(result.type == "register"){
                error_form = "error-register";
                $("#error-register").empty();
              }else{
                error_form = "error-login";
                $("#error-login").empty();
              }
              for(var i = 0; i < error.length; i++){
                var para = document.createElement("p");
                var node = document.createTextNode(error[i]);
                para.appendChild(node);
                var element = document.getElementById(error_form);
                element.appendChild(para);
                 ...
                 swal({
                    ...
                    confirmButtonColor: "#ff961e",                   
                    confirmButtonText: "Đăng ký trung tâm",                   
                    cancelButtonText: "x",                   
                    closeOnConfirm: false,                   
                    closeOnCancel: false },                   
                    function(isConfirm){                   
                    if (isConfirm){                   
                       $(location).attr('href',  "become-agency");
                    }
                    else {
               ...
               if (result.popup == 2) {
                 swal({
                    ...
                    showCancelButton: true,                   
                    confirmButtonColor: "#ff961e",                   
                    confirmButtonText: "Xem gia sư phù hợp",                   
                    cancelButtonText: "x",                   
                    closeOnConfirm: false,                   
                    closeOnCancel: false },                   
                    function(isConfirm){                   
        	...
        	alert("Xin vui lòng nhập đủ các thông tin bắt buộc");
        	return false;
        }    	
    });
    var date_input=$('#day-start'); //our date input has the name "date"
    ...
    //$(".action-class-form input").prop("disabled",true);
    $('#offline-type').click(function() {
        if (!$(this).is(':checked')) {
            $(".class-address").hide();
        }else{
			$(".class-address").show();
    ...
    $('.read-more-trigger').popover({
        ...
        content: function () {
            return "Thêm các thông tin về: Giới tính giáo viên, Ngày học dự kiến,Số học viên,Số buổi 1 tuần và buổi có thể học";
        },
        container: 'body',
        placement: 'auto'
        ...
        $('.read-more-trigger').not(_this).popover('hide');
        ...
        counter = setTimeout(function(){
            ...
            $(".popover").on("mouseleave", function () {
                $('.read-more-trigger').popover('hide');
            });
        }, 400);
   ...
   //poppover gia lop hoc
   var counter;
   $('#create-class-form #price_cost').popover({
        trigger: 'manual',
        animation: false,
        html: true,
        title: function () {
        ...
        content: function () {
            return "Mức học phí phổ biến hiện tại là: 200,000 - 300,000 vnd/buổi (2h)";
        },
        container: 'body',
        placement: 'auto'
    }).on("mouseenter",function () {
...
function validate_form(){
	var fields = ["name_class","number_student","price_cost","phone_class","subject-class","introduce-class"];
  var missing = [];
  var result = true;
	for(var i=0;i<fields.length;i++){
		if($("#"+fields[i]).val() == ""){
      $("#"+fields[i]).addClass('missingValue');
			missing.push(fields[i]);
		}else{
      $("#"+fields[i]).removeClass('missingValue');
    }
	}
  //
  if ($('#term-of-user').size() && !$('#term-of-user').is(':checked')) {
		$('#text-term-of-use').addClass('missingValueText');
    result =  false;
  }else if($('#term-of-user').is(':checked'))
	{
		$('#text-term-of-use').removeClass('missingValueText');
	}
  //
  if($("#offline-type").is(':checked')){
    if($("#select-country-class").val() == ""){
      $("#select-country-class").addClass('missingValue');
      result =  false;
    }else{
      $("#select-country-class").removeClass('missingValue');
    }
    if($("#address-class").val() == ""){
      $("#address-class").addClass('missingValue');
      result =  false;
    }else{
      $("#address-class").removeClass('missingValue');
    }
  }
  ...
  if(missing.length >= 1){
	...
	if (!$("#online-type").is(':checked') && !$("#offline-type").is(':checked')){
		result =  false;
	}
...
function ajax_upload_image(element){
  ...
  handleDrop = function(files){
          ...
          if (file.type.match('image.*')) {
              resizeImage(file, 312, function(result,fileImg) {
                //$('#'+imgShow).attr('src', result);
                if($("#preview_img_class")){
                  $("#preview_img_class").remove();
                }
                $('#preview_image_class_show').attr('src', result);
                //$('.resultImageWrapper').show();
                dataUrl = fileImg;
                $(".box").append( "<div class='loading-small'></div>" );
                var data = new FormData();                   
                data.append('files', dataUrl);
                $fid_old = $("#fid_img_lop_hoc").val();
                data.append('fid_img_class',$fid_old);        
               
                $.ajax({
                    url: '/upload-image-class',
                    data: data,
                    dataType: 'json',
                    cache: false,
                    contentType: false,
                    ...
                    type: 'POST',
                    success: function(result){
                      $("#fid_img_lop_hoc").val(result.result);
                      
                      $(".loading-small").remove();
                      //$(".action-class-form input").prop("disabled",false);
                    }
                });
  ...
  if(!allowedExtensions.exec(filePath)){
      alert('Chỉ được upload ảnh với định dạng .jpeg/.jpg/.png/.gif .');
      fileInput.value = '';
      return false;
  }else{
      //Image preview
