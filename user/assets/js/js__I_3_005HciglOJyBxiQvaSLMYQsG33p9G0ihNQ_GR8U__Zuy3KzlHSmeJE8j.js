...
jQuery(document).ready(function () {
    if (jQuery('#search-global-mobile').length > 0) {
        jQuery('#search-global-mobile').change(function () {
            jQuery('#search-global-mobile').submit();
        });
    }
    // search header
    jQuery('.search-header .btn-search').click(function () {
        jQuery(this).parents('.navbar-form').toggleClass('open-drop');
    });

    // dropdown header
    jQuery('.dropdown.notification .dropdown-menu .list').mCustomScrollbar();
    jQuery('.navbar-default .navbar-nav.navbar-right > li.dropdown .dropdown-menu').on('click', function () {
        event.stopPropagation();
    });
    ...
    // tao khoa hoc
    jQuery('.custom-select select').niceSelect();
    // e: tao khoa hoc
    ...
    jQuery('.modal-image-gallery .slider').slick({
        dots: false,
        infinite: false,
        speed: 300,
        slidesToShow: 1,
    ...
    jQuery('.folder-big-content .header-folder nav .toggle').click(function () {
        jQuery('.folder-big-content .header-folder nav ul').toggle();
    });
    jQuery('.slider-other-blog').slick({
        dots: true,
        arrows: false,
        infinite: false,
        speed: 300,
    ...
    jQuery('.nav-search h3').click(function () {
        jQuery(this).parents('.nav-search').toggleClass('open-search');
    });
    /* e: blog */

    /* tuyen dung */
    jQuery('.filter-box h3').click(function () {
        jQuery(this).parent().toggleClass('open-checkbox');
    });
    /* e: tuyen dung */
    /* js for version 1.2 */
    jQuery('.custom-scroll').mCustomScrollbar();

    jQuery('.main-slider .slider').slick({
        arrows: false,
        dots: false,
        autoplay: true,
        slidesToShow: 1,
    ...
    jQuery('.main-slider .slider-nav').slick({
        slidesToShow: 5,
        slidesToScroll: 1,
    ...
    jQuery('.main-slider .slider').on("afterChange", function (event, slick, currentSlide) {
        // Remove current class from second slider
        jQuery('.main-slider .slider-nav').find(".slick-current").removeClass("slick-current");

        // Add current class to the current slide
        jQuery('.main-slider .slider-nav').find(".slick-slide").eq(currentSlide).addClass("slick-current");
    });


    jQuery('.articles-events .tags a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
        jQuery('.articles-events .main-blog .slider').slick('setPosition');
    });

    jQuery('.content-category .header-block button').click(function () {
        jQuery('.content-category .nav-tabs').toggle();
    });

    jQuery('.nav-category .box-search h4').click(function () {
        jQuery(this).parent().toggleClass('open');
    });

    jQuery('.box-main-detail .big-img-slider').slick({
        arrows: false,
        autoPlay: true,
        fade: true,
        slidesToShow: 1,
        slidesToScroll: 1,
        asNavFor: '.box-main-detail .nav-slider',
        // adaptiveHeight:true
    });

    jQuery('.box-main-detail .nav-slider').slick({
        slidesToShow: 6,
        slidesToScroll: 1,
        asNavFor: '.box-main-detail .big-img-slider',
        dots: true,
        centerMode: true,
        focusOnSelect: true,
        //autoPlay:true,
    ...
    jQuery('.block-slider .home-teacher-slider').slick({
        slidesToScroll: 1,
        slidesToShow: 4,
        dots: false,
        arrows: true,
    ...
    }).addClass('slider-proccess');
    jQuery('.add').on('click', function () {
        var $qty = jQuery(this).closest('.number-product').find('input');
        var currentVal = parseInt($qty.val());
        if (parseInt($qty.attr('max')) > currentVal) {
            if (!isNaN(currentVal)) {
    ...
    //Custom handle buy product on node detail by GD
    if (jQuery('.node-custom-buy').length) {
        jQuery('.node-custom-buy').once('change_qty').on('change, keyup', 'input.qty', function () {
            var target = jQuery(this).attr('data-id');
            jQuery(target).val(this.value);
        })
        jQuery('.node-custom-buy').once('click_buy').on('click', '.buy-btn', function (e) {
            var nid = jQuery(this).attr('nid');
            var $this = jQuery(this);
            jQuery.get('/check-stock/' + nid + '?type=json', function (data) {
                var target = $this.attr('data-id');
                if (parseInt(data) > 0) {
                    var qty_target = jQuery('.node-custom-buy').find('input.qty').attr('data-id');
                    jQuery(qty_target).val(jQuery('.node-custom-buy').find('input.qty').val());
                    if (parseInt(data) >= $this.parents('.node-custom-buy').find('input.qty').val()) {
                        jQuery(target).trigger('click');
                    } else {
                        jQuery('#node-custom-msg').html('<span class="text-warning">Còn ' + data + ' sản phẩm</span>');
                        $this.parents('.node-custom-buy').find('input.qty').attr('max', data);
                    }
                } else {
                    jQuery('#node-custom-msg').html('<span class="text-red">Hết hàng</span>');
                }
            });
        })
    }

    jQuery('.info-product-for-cart.order .custom-scroll').mCustomScrollbar();
    /* js for version 1.2 */
    ...
    jQuery(window).scroll(function () {
        if (jQuery(this).scrollTop()) {
            jQuery('body').addClass('bd-fixed-untop');
        } else {
            jQuery('body').removeClass('bd-fixed-untop');
        }

        // fix content

        // var w_scroll = jQuery(this).scrollTop()
        //            content_top = (jQuery('.home-category').length) ? jQuery('.home-category').offset().top : 0;
        //
        //        var top_1 = w_scroll+ 50 - content_top;        
        //        var top_admin_1 = w_scroll+50 - content_top + 29;
        //        if (content_top - w_scroll <= 50){
        //            jQuery('body').addClass('bd-fixed-content');            
        //            jQuery('.nav-category.fixed .detail-nav').css({
        //                top: top_1
        //            });
        //            jQuery('.content-category.fixed .header-block').css({
        //                top: top_1
        //            });
        //            jQuery('.content-category.fixed .tab-content .tab-pane .view-filters .form-select').css({
        //                top: top_1
        //            });
        //          
        //
        //            // admin            
        //            jQuery('.admin-menu .nav-category.fixed .detail-nav').css({
        //                top: top_admin_1
        //            });
        //            jQuery('.admin-menu .content-category.fixed .header-block').css({
        //                top: top_admin_1
        //            });
        //            jQuery('.admin-menu .content-category.fixed .tab-content .tab-pane .view-filters .form-select').css({
        //                top: top_admin_1
        //            });
        //        }
        //        else{
        //            jQuery('body').removeClass('bd-fixed-content');
        //            jQuery('.content-category.fixed .tab-content .tab-pane .view-filters .form-select').css({
        //                top:'-47px'
        //            });
        //        }

        // var top_2 = w_scroll+50 - content_top;
        //        if (jQuery('.fixed.right-col-v1-2 .statics-course.statics-group').length>0){
        //        	var content_top_2 = jQuery('.fixed.right-col-v1-2 .relative.detail').offset().top;
        //        	console.log('content 2:'+content_top_2);
        //	        if (content_top_2 - w_scroll <= 50){
        //	        	jQuery('body').addClass('bd-fixed-content-detail');                
        //	    		jQuery('.fixed.right-col-v1-2 .statics-course.statics-group').css({
        //	                top: top_2
        //	            });
        //
        //	            //admin
        //                var top_admin_2 = w_scroll+50 - content_top + 29;
        //	            jQuery('.admin-menu .fixed.right-col-v1-2 .statics-course.statics-group').css({
        //	                top: top_admin_2
        //	            });
        //	        }
        //	        else{
        //	            jQuery('body').removeClass('bd-fixed-content-detail');
        //	        }
        //        }
    ...
    // if (jQuery('.group-study-detail .detail-content').length>0){
    //        var h_content = jQuery('.group-study-detail .detail-content .body-block').outerHeight(true);
    //        console.log(h_content);
    //        jQuery('.group-study-detail .content-facebook .body-block').css({
    //            'max-height':h_content,
    //            'overflow-x':'hidden',
    //            'overflow-y':'auto'
    //        });
    //   }
...
;/*})'"*/
////////////////////////////////////////////////////////////////////////////////
/////////////////CÁC HÀM THƯỜNG ĐƯỢC DÙNG CHUNG TRONG HỆ THỐNG BLACASA//////////
/////////////////COPYRIGHT: BLACASA VIETNAM JSC.////////////////////////////////
////////////////////////////////////////////////////////////////////////////////

/*
    QUY TẮC ĐẶT TÊN: các hàm dùng chung bắt đầu bằng 'bla' rồi đến tên Hàm 
    theo quy tắc CammelRule, ví dụ: blaTenCuaHam
*/
...
(function ($) {
    ...
    Drupal.behaviors.blacasa = {
        attach: function (context, settings) {
                ...
                var cv = setInterval(function () {
                    if (window.countv > 5) {
                        ...
                        if ($(window).width() > 1200) {
                            $('.col-vjustify').height('auto').vjustify();
                            window.countv = window.countv + 1;
                        }
                    }
                }, 1000);
            });
            ...
            //search header click
            //$('.reload-search #custom-search-object-form  select').change(function () {
            //        var dia_diem = $('#custom-search-object-form select[name="dia_diem"]').val();
            //        var $mon = $('#custom-search-object-form select[name="mon_hoc"]').val();
            //        window.location.href = Drupal.settings.basePath + Drupal.settings.pathPrefix + doi_tuong + "/" + dia_diem;
            //      });
            //slider
            $('.main-blog ul.event-home').owlCarousel({
                items: 1,
                itemsDesktop: [1199, 1],
            ...
            //click menu dieu le
            $('#dieu-le-menu a').click(function () {
                var _width = $(window).width();
                if (_width <= 600) {
                    $('#dieu-le-menu').removeClass('in');
                }
            });
            //click menu blog
            var _width = $(window).width();
            if (_width <= 600) {
                $('.header-blog ul.navbar-nav>li.dropdown>a').attr('data-toggle', 'dropdown');
                $('.header-blog ul.navbar-nav>li.dropdown>a').attr('aria-expanded', 'true');
            }
            //marquee
            $('.divmarquee').marquee({
            ...
            //
            $('.link-scroll a:not(.link-process)').click(function () {
            ...
            }).addClass('link-process');

            $('.page-info .link-scroll a:not(.link-process)').click(function () {
                var _id = $(this).attr('href');
                $("html, body").stop().animate({
                    scrollTop: $(_id).offset().top - 200
                }, 'slow');
            ...
            $('.save-favorite').click(function () {
                ...
                var element = $(this);
                //var items = _val.split('-');
                $.ajax({
                    type: 'post',
                    url: '/save-favorite',
                    //contentType: "application/json; charset=utf-8",
                    dataType: 'html',
                    data: { id: id, uid: uid, type: _type },
                    success: function (data) {
                        var obj = jQuery.parseJSON(data);
                        if (obj.status == 1) {
                            $('.text', element).text('Đã lưu');
                            $(element).removeClass('favorite-gray');
                            $(element).addClass('favorite-blue');
                        } else {
                            $('.text', element).text('Lưu');
                            $(element).removeClass('favorite-blue');
                            $(element).addClass('favorite-gray');
                        }
            ...
            //duyet tham gia nhom
            $('.node-thanh-vien .duyet-yeu-cau').click(function () {
                var nid = $(this).attr('data-nid');
                var action = $(this).attr('data-action');

                $.ajax({
                    type: 'post',
                    url: '/duyet-yeu-cau-tham-gia',
                    //contentType: "application/json; charset=utf-8",
                    dataType: 'html',
            ...
            $('input[type="rang"]').each(function () {
                var element_key = $(this).attr('key');
                $(this).rangeslider({
                    polyfill: false,
                    onSlide: function (position, value) {
                        $('input[name="criteria_' + element_key + '"]').val(value);
                    },
                });
            });

            //Form tao lop hoc
            if ($('.node-studygroup-form #edit-field-subject-und').size()) {
                $('.node-studygroup-form #edit-field-subject-und', context).chosen({
                    width: "100%",
                    placeholder_text_multiple: 'Lựa chọn',
                    placeholder_text_single: 'Lựa chọn',
                    no_results_text: 'Không có kết quả'
                });
            }
            // Trang home
            $('#articles-events-home ul.tags li').click(function () {
                var tid = $('a', $(this)).attr('aria-controls');
                $.ajax({
                    type: 'post',
                    url: '/ajax-article-home',
                    //contentType: "application/json; charset=utf-8",
                    dataType: 'json',
                    data: { tid: tid },
                    success: function (data) {
                        $('#articles-events-home-content').hide().html(data.html).fadeIn({ duration: 2000 });
                    }
                })
            });
            ...
            if ($('#studygroup-node-form #edit-field-calendar-und').size()) {
                $('#studygroup-node-form #edit-field-calendar-und .form-type-checkbox').each(function () {
                    if ($('input', $(this)).is(':checked')) {
                        $('label', $(this)).addClass("lbl-active");
                    }
                });
                $('#studygroup-node-form #edit-field-calendar-und .form-type-checkbox label').click(function () {
            ...
            // Check calendar
            if ($('.row-calendar-check li label').size()) {
                //$('.row-calendar-check li label').click(function () {
                //          if($(this).hasClass("lbl-active")){
                //              $(this).removeClass("lbl-active");
                //          }else{
                //              $(this).addClass("lbl-active");
                //          }
                //      });
                $('.row-calendar-check li input').change(function () {
                    var id = $(this).attr('id');
                    if ($(this).is(':checked')) {
                        $('label[for="' + id + '"]').addClass("lbl-active");
                    } else {
                        $('label[for="' + id + '"]').removeClass("lbl-active");
                    }
                });
            }
            // CHECK SELECT TAG

            $('.subject-tag label').click(function () {
                if ($(this).hasClass("lbl-active")) {
                    $(this).removeClass("lbl-active");
                } else {
                    $(this).addClass("lbl-active");
...
// JS xử lý đăng ký trở thành giáo viên
function become_teacher() {
    ...
    $.ajax({
        ...
        success:
            function (result) {
                $(".loading").remove();
                window.location.href = "/user/" + result.result + "/account";
            }
    });
}
// Hàm resize ảnh trước khi upload
// callback hàm xử lý sau khi resize
function resizeImage(file, size, callback) {
    ...
    fileTracker.onload = function () {
        ...
        image.onload = function () {
            ...
            */
            ...
            callback(canvas.toDataURL("image/JPEG"), resizedImage);
        };
    ...
    fileTracker.onabort = function () {
        alert("Việc tải file đã bị huỷ.");
    }
    fileTracker.onerror = function () {
        alert("Đã có lỗi xảy ra trong quá trình đọc file.");
    }
}
...
////////////////////////////////////////////////////////////////////////////////
/*@Summary: định dạng số, có dấu phẩy phân hàng nghìn và dấu chấm phân hàng thập phân
* Note: chuỗi số input nhập vào sẽ theo format Vietnam và US, nghĩa là dấu phẩy ',' dùng để
* phân hàng nghìn, và dấu chấm '.' để phân phần thập phân.
*@param nStr: chuỗi số input để định dạng
*@param nDeciamal: số chữ số phần thập phân, mạc định là 2
*@author: Tuan Nam Nguyen
*/
function blaFormatNumber(nStr, nDecimal = 2) {
    //Step 0: check valid number
    var nNumber = nStr;
    nNumber = nNumber.replace(/,/g, '');

    //STep 1: round
    if (nDecimal >= 0)
        nNumber = Math.round(nNumber * Math.pow(10, nDecimal)) / Math.pow(10, nDecimal);

    //Step 2: format with comma and point
    nNumber += '';
    var x = nNumber.split('.');
    var x1 = x[0];//phan truoc thap phan
